...
public class PitchBalance extends CommandBase {
  ...
  public void execute() {
    ...
    swerve.driveBackward(AutoConsts.DRIVE_TRANSLATION_SPEED * 0.7);
    
    // Checks if pitch is greater than -13.5
    if (swerve.getPitch() > 12){
      readyToEnd = true;
    }
  }
  ...
  public void end(boolean interrupted) {
    SmartDashboard.putString("Current Command", "pitch finish");
  ...
  public boolean isFinished() {
    ...
    return (readyToEnd && swerve.getPitch() < 9) /*|| Timer.getMatchTime()<1.5*/; //|| (-5<swerve.getPitch() && swerve.getPitch()<5 && Timer.getMatchTime() < 1);
  }
}
